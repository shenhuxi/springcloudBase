$(function() {
  ...
  function onSuccess(data) {
    ...
    function addApiKeyAuthorization(security) {
      ...
      // }
      var apiKeyVehicle = 'header';
      var apiKeyName = 'Authorization';

      //或取前端保存的token
      var access_token = localStorage.getItem("token") || sessionStorage.getItem("token");
      var apiKey = access_token || '';
      if (apiKey && apiKey.trim() != "") {
        var apiKeyAuth = new SwaggerClient.ApiKeyAuthorization(apiKeyName, "Bearer " + apiKey , apiKeyVehicle);
        window.swaggerUi.api.clientAuthorizations.add("bearer", apiKeyAuth);
        log("added key " + apiKey);
      } else {
          log("dx-authenticationtoken is null");
      }
    }
